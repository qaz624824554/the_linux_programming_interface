...
main(int argc, char *argv[])
{
    ...
    /* Set 'msgh' fields to describe the ancillary data buffer.
       ...
       The 'optControlMsgSize' value (specified as a command-line option) can
       be used to artificially limit the size of the received ancillary data.
       This can be used to demonstrate that when the buffer size is too small,
       the list of received file descriptors is truncated, and the excess file
       descriptors are automatically closed. */
    ...
    for (struct cmsghdr *cmsgp = CMSG_FIRSTHDR(&msgh);
        ...
        case SCM_RIGHTS:        /* Header containing file descriptors */
            ...
            /* Allocate an array to hold the received file descriptors, and
               copy file descriptors from cmsg into array. */
